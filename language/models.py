...
class emel_Transformer(nn.Module):
    """ Transformer with Self-Attentive Blocks"""
    def __init__(self, cfg):
        super().__init__()
        print('in transformer emel')

        self.embed = Embeddings(cfg)
        self.blocks = nn.ModuleList([Block(cfg) for _ in range(cfg.n_layers)])
        self.layers_with_emel = [4, 5, 6, 7]
        self.emels = nn.ModuleList([EmeL()  # EmeL([batch, max_len, cfg.dim])
                                    for _ in self.layers_with_emel])
    ...
    def forward(self, x, seg, mask):
        ...
        for block in self.blocks:
            ...
            if idx in self.layers_with_emel:
                h = self.emels[self.layers_with_emel.index(idx)](h)
            idx += 1
        return h
