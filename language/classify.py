...
class CsvDataset(Dataset):
    ...
    def __init__(self, file, pipeline=[]): # cvs file and pipeline object
        ...
        if 'STSB' in file:
            self.tensors[-1] = torch.tensor([float(x[-1]) for x in data], dtype=torch.float)
...
class QQP(CsvDataset):
    ...
    def get_instances(self, lines):
        for line in itertools.islice(lines, 1, None): # skip header
            yield line[-1], line[-3], line[-2] # label, text_a, text_b
...
class STSB(CsvDataset):
    ...
    def get_instances(self, lines):
        for line in itertools.islice(lines, 1, None): # skip header
            yield str(float(line[-1])/5.0), line[-3], line[-2] # label, text_a, text_b
...
class CoLA(CsvDataset):
    ...
    def get_instances(self, lines):
        for line in itertools.islice(lines, 1, None): # skip header
            yield line[-3], line[-1], None # label, text_a, text_b
...
def dataset_class(task):
    ...
    table = {'mrpc': MRPC, 'mnli': MNLI, 'QQP': QQP, 'STSB': STSB, 'CoLA': CoLA, 'QNLI': QNLI, 'RTE': RTE, 'SST2': SST2}
    return table[task]
...
class emel_Classifier(nn.Module):
    """ Classifier with Transformer """
    def __init__(self, cfg, n_labels):
        super().__init__()
        self.transformer = models.emel_Transformer(cfg)
...
def main(task='mrpc',
    ...
    if 'emel' not in mode:
        ...
        model = emel_Classifier(model_cfg, len(TaskDataset.labels))
    criterion = nn.CrossEntropyLoss()

    trainer = train.Trainer(cfg,
                            model,
                            data_iter,
                            optim.optim4GPU(cfg, model, lr=lr),
                            save_dir, get_device())
    print(mode)
    if mode == 'train':
        ...
        def evaluate(model, batch):
            ...
            if 'STSB' in data_file:
                label_id = torch.tensor((label_id>0.5), dtype=torch.long)
    ...
    elif 'emel' in mode and 'train' in mode:
        def get_loss(model, batch, global_step): # make sure loss is a scalar tensor
            input_ids, segment_ids, input_mask, label_id = batch
            logits = model(input_ids, segment_ids, input_mask)

            if 'STSB' in data_file:
                # use soft cross entropy loss when training with STSB
                logits = nn.functional.softmax(logits, dim=1)
                loss = (-label_id * torch.log(logits[:, 1]) - (1-label_id) * torch.log(logits[:, 0])).mean()
            else:
                loss = criterion(logits, label_id)
            return loss
