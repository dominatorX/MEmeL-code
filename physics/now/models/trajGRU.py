...
def activate(act_type, filters, layer):
    if act_type is not None:
        if act_type == 'pacs':
            layer = torch.split(layer, filters, dim=1)
            return torch.cat([layer[0], layer[1] ** 2 / 2, layer[2] ** 3 / 6, layer[3] ** 4 / 24], dim=1)
        else:
...
class BaseConvRNN(nn.Module):
    def __init__(self, num_filter, b_h_w,
                 ...
                 cnn_act_type=torch.tanh,
                 rnn_act_type=torch.tanh,
                 prefix='BaseConvRNN'):
...
class TrajGRU(BaseConvRNN):
    ...
    def __init__(self, input_channel, num_filter, b_h_w, zoneout=0.0, L=5,
                 ...
                 rnn_act_type=cfg.MODEL.RNN_ACT_TYPE, cnn_act_type=cfg.MODEL.CNN_ACT_TYPE,
                 norm=None):
        super(TrajGRU, self).__init__(num_filter=num_filter,
                                      ...
                                      cnn_act_type=cnn_act_type,
                                      rnn_act_type=rnn_act_type,
                                      prefix='TrajGRU')
        self._L = L
        self._zoneout = zoneout
        self._norm1, self._norm2 = (None, None) if norm is None else (norm([32]), norm([num_filter]))
    ...
    def _flow_generator(self, inputs, states):
        ...
        f_conv1 = activate(self._cnn_act_type, self._num_filter//4, f_conv1)
        if self._norm1 is not None:
            f_conv1 = self._norm1(f_conv1)
    ...
    def forward(self, inputs=None, states=None, seq_len=cfg.HKO.BENCHMARK.IN_LEN):
        ...
        for i in range(seq_len):
            ...
            if i2h_slice is not None:
                ...
                new_mem = activate(self._rnn_act_type, self._num_filter//4,
                                   i2h_slice[2][i, ...] + reset_gate * h2h_slice[2])
                if self._norm1 is not None:
                    new_mem = self._norm2(new_mem)
            else:
